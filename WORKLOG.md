...
- 兼职板块支持计时 + 时薪算钱
- 临时事件可选计时
- 日历切换（收进右上角 📅 按钮，点击弹出，有记录打点，可切换任意日期）
...
- UI 设计重构（frontend-design 技能）：每日主题印章 + 衬线展示字体 + 语义色条 + 精选错峰入场
...
- 清单书籍联想检索（`GET /api/book-search`，Google Books 优先 + Open Library 兜底，输入书名/作者自动联想）
